# chimeric_identity/identity_registry.py
# ============================================================================
# IDENTITY REGISTRY - Simple Person-to-Gallery Mapping
# ============================================================================
#
# Purpose:
#   Map between face gallery IDs and gait gallery IDs for the same person.
#   This is the MISSING LINK that allows chimeric fusion to know when
#   face and gait are referring to the same person.
#
# Design Philosophy:
#   - SIMPLE: Just a dictionary with JSON persistence
#   - LIGHTWEIGHT: No complex state machines or accumulators
#   - SINGLE SOURCE OF TRUTH: One file maps all identities
#
# Usage:
#   During enrollment: Register both face_id and gait_id for a person
#   During recognition: Look up to see if face_id and gait_id match
...
class PersonRecord:
    ...
    """
    display_name: str           # Human-readable name (e.g., "Marildo")
    face_gallery_id: Optional[str] = None   # ID in face gallery (e.g., "p_0001")
    gait_gallery_id: Optional[str] = None   # ID in gait gallery (e.g., "Marildo")
...
class IdentityRegistry:
    ...
    def __init__(self, registry_path: Optional[Path] = None):
        ...
        # Person records by display name
        self._persons: Dict[str, PersonRecord] = {}
        
        # Reverse lookup indexes (for O(1) lookup by gallery ID)
        self._face_to_person: Dict[str, str] = {}  # face_id -> display_name
        self._gait_to_person: Dict[str, str] = {}  # gait_id -> display_name
    ...
    # ========================================================================
    # REGISTRATION (Used during enrollment)
    # ========================================================================
    ...
    # ========================================================================
    # LOOKUP (Used during recognition/fusion)
    # ========================================================================
    ...
    def are_same_person(
        ...
        if face_person is None or gait_person is None:
            # One or both IDs not registered - check if they're literally equal
            # (for backwards compatibility when same naming is used)
