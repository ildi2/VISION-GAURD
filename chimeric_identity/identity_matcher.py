# chimeric_identity/identity_matcher.py
# ============================================================================
# INTELLIGENT IDENTITY MATCHER
# ============================================================================
#
# Solves the critical problem: Face gallery and gait gallery use DIFFERENT 
# naming schemes. This module provides intelligent matching WITHOUT modifying
# the face or gait engine logic.
#
# Face Gallery IDs:  "p_0007" with name "marildo cani"
# Gait Gallery IDs:  "marildo" (uses name as ID)
#
# This matcher finds: "marildo cani" ↔ "marildo" = SAME PERSON
#
# DESIGN PRINCIPLES:
# ==================
# 1. READ-ONLY: Never modify face or gait galleries
# 2. NON-INVASIVE: Works as a bridge layer, not replacement
# 3. SMART MATCHING: Handles name variations, partial matches
# 4. EXPLICIT MAPPING: Creates clear, auditable identity_registry.json
...
class FaceIdentity:
    ...
    person_id: str      # e.g., "p_0007"
    name: str           # e.g., "marildo cani"
    category: str       # e.g., "resident"
    num_templates: int
...
class IdentityMatcher:
    ...
    def _calculate_match(
        ...
        # "marildo" in "marildo cani" → high confidence
        if gait_norm in face_norm:
            # Gait ID is a substring of face name
            # Confidence based on how much of face name is matched
            ratio = len(gait_norm) / len(face_norm)
            confidence = 0.85 + (ratio * 0.10)  # 0.85-0.95
            return min(confidence, 0.95), "partial"
        
        if face_norm in gait_norm:
            # Face name is a substring of gait ID (less common)
            ratio = len(face_norm) / len(gait_norm)
            confidence = 0.80 + (ratio * 0.10)
            return min(confidence, 0.90), "partial"
        
        # Strategy 3: WORD match
        # "marildo cani" vs "marildo" → check if first name matches
        face_words = face_norm.split()
        gait_words = gait_norm.split()
        
        for fw in face_words:
            for gw in gait_words:
                if fw == gw and len(fw) >= 3:  # Avoid matching short words
                    # Word match - good confidence
                    return 0.85, "word"
    ...
    def _string_similarity(self, s1: str, s2: str) -> float:
        ...
        # Similarity is LCS / average length
...
def auto_sync_registry(
    ...
    for match in matches:
        ...
        if not dry_run and match.confidence >= min_confidence:
            # Determine display name (prefer face name, it's usually more complete)
...
def manual_link(
    ...
    # Get display name from face gallery if not provided
