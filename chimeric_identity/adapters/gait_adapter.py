# chimeric_identity/adapters/gait_adapter.py
# ============================================================================
# GAIT ADAPTER - Non-Invasive Wrapper for Gait Engine
# ============================================================================
#
# Purpose:
#   Convert GaitEngine outputs into normalized GaitEvidence for chimeric fusion.
#   READ-ONLY access to existing gait subsystem (zero modifications).
#
# Design Philosophy:
#   - Stateless: No side effects on gait engine state
#   - Defensive: Handle missing/incomplete gait data gracefully
#   - Transparent: Preserve gait quality and state information
#   - Temporal Aware: Gait needs 1000ms+ of data before decisions
#
# Input: IdentityDecision from GaitEngine + GaitTrackState
# Output: GaitEvidence (normalized schema)
#
# Key Innovation:
#   Maps GaitState (COLLECTING/EVALUATING/CONFIRMED/UNSURE) to EvidenceStatus
#   Extracts confidence from similarity + margin + streak mechanics.
...
class GaitAdapter:
    ...
    def adapt_decision(
        ...
        # Get gait track state
        if gait_track_state is None:
            # No state → no evidence (shouldn't happen)
            return None
        
        # Extract state machine info
        gait_state = gait_track_state.state
        gait_reason = gait_track_state.reason
        q_seq = gait_track_state.q_seq  # Sequence quality
        
        # Extract identity info from decision
        if gait_identity_decision is None:
            # No decision → use state info only
        ...
        # Extract margin (from extra field or state)
        margin = 0.0
        ...
        # Map gait state to evidence status
        evidence_status = GaitAdapter._map_gait_state(gait_state, gait_reason)
        
        # Calculate gait confidence (different from face)
        ...
        gait_ev = GaitEvidence(
            ...
            timestamp=now,
            freshness_window_sec=3.0,  # Gait is slower, 3s window
            extra={
    ...
    def _map_gait_state(
        ...
        if state_upper == "COLLECTING":
            ...
            # During EVALUATING, might be TENTATIVE if sim is decent
            if gait_reason and "CONFIRM" in gait_reason.value:
                return EvidenceStatus.CONFIRMED_WEAK  # Ready to confirm
            else:
                return EvidenceStatus.EVALUATING  # Still evaluating
    ...
    def _extract_gait_confidence(
        ...
        """
        # Margin weight: 0.05 margin is minimum safe value
        if margin >= 0.10:
            margin_weight = 1.0  # Excellent margin
        elif margin >= 0.05:
            margin_weight = 0.5 + (margin / 0.10)  # Scale 0.5-1.0 for margin 0-0.10
        else:
            margin_weight = 0.2  # Poor margin (risky)
        
        # Quality weight (more stringent than face)
        quality_weight = (quality ** 1.0)  # Linear, not sqrt
        
        # Sequence length bonus (longer = more confident)
        # 30 frames → 1.0 multiplier
        # 60 frames → 1.05 multiplier
        # 120+ frames → 1.10 multiplier (cap)
        seq_bonus = min(1.10, 1.0 + (sequence_length - 30) / 300)
        
        # Streak bonus: each confirm adds 10% confidence
