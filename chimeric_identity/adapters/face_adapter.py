# chimeric_identity/adapters/face_adapter.py
# ============================================================================
# FACE ADAPTER - Non-Invasive Wrapper for Face Identity Engine
# ============================================================================
#
# Purpose:
#   Convert IdentityEngine outputs into normalized FaceEvidence for chimeric fusion.
#   READ-ONLY access to existing face subsystem (zero modifications).
#
# Design Philosophy:
#   - Stateless: No side effects on face engine state
#   - Defensive: Handle missing/malformed inputs gracefully
#   - Transparent: Preserve audit information from original decision
#   - Efficient: Minimal processing overhead
#
# Input: IdentityDecision from FaceIdentityEngine
# Output: FaceEvidence (normalized schema)
#
# Key Innovation:
#   Maps binding_state (UNKNOWN/PENDING/CONFIRMED_WEAK/STRONG/STALE) and
#   distance-bands (strong/weak/none) into unified EvidenceStatus.
...
class FaceAdapter:
    ...
    def adapt_decision(
        ...
        # No face decision → no face evidence
        ...
        # Extract core fields from IdentityDecision
        ...
        # Extract gallery distance (for margin calculation)
        distance = identity_decision.distance or 0.0
        similarity = max(0.0, 1.0 - distance)  # distance → similarity
        
        # Extract second-best match info if available (for margin)
        margin = 0.0
        second_best_id = None
        ...
        # Map binding_state to EvidenceStatus
        evidence_status = FaceAdapter._map_binding_state(binding_state)
        
        # Quality gate: If quality is very low, downgrade status
        # But still return evidence for audit trail
        if quality < 0.45:  # Min quality threshold
            evidence_status = EvidenceStatus.UNKNOWN
        
        # Extract source auth info if present
        ...
        face_ev = FaceEvidence(
            ...
            timestamp=now,
            freshness_window_sec=2.0,  # Face evidence is fresh for 2s
    ...
    def extract_confidence_from_similarity(
        ...
        """
        # Quality weight: sqrt for gentler penalty
        quality_weight = (quality ** 0.5)
        
        # Margin weight: reward good margin, penalize poor margin
        if margin >= 0.10:
            margin_weight = 1.0  # Good margin
        else:
            margin_weight = margin / 0.10  # Scale 0-1 for margin 0-0.10
            margin_weight = max(0.1, margin_weight)  # Never below 0.1
