...
# ---------------------------------------------------------------------------
# Config and public data structures
# ---------------------------------------------------------------------------
...
class _InternalTrack:
    ...
    def predict(self) -> None:
        ...
        # Small decay so velocity does not explode
        self.velocity *= 0.9
    ...
    def update(
        ...
        """
        # Update velocity estimate (difference between new and old bbox)
        self.velocity = new_bbox - self.bbox

        # Smooth update of bbox (can tune if needed)
        self.bbox = new_bbox
        ...
        # Update appearance embedding via exponential moving average
...
class OCSortTracker:
    ...
    def update(
        ...
        # 2) Build cost matrix between predicted tracks and detections
        ...
        for track_idx, det_idx in matches:
            ...
            # Confirm track once it has enough hits
        ...
        # 6) Export public Track objects:
        #    Only tracks that are confirmed and updated in this frame.
        output_tracks: List[Track] = []
        for trk in self.tracks:
            if not trk.confirmed:
                continue
            if trk.time_since_update != 0:
                # skip tracks that were not associated with a detection this frame
                continue
    ...
    def _associate(
        ...
        # Build score matrix (tracks x detections)
            ...
            for d_idx, det in enumerate(detections):
                ...
                if (
                    ...
                    cos_sim = cosine_similarity(
                        trk.appearance, appearance_features[d_idx]  # type: ignore[arg-type]
                    )
                    # Normalize cos_sim from [-1, 1] to [0, 1]
