...
class SchedulerConfig:
    ...
    # Priority weights (higher = more likely to be scheduled)
    ...
    # Time decay (older tracks get higher priority)
    time_decay_rate: float = 20.0  # priority_delta per second
    
    # FPS thresholds for budget allocation
    fps_high: float = 15.0  # All faces
    fps_medium: float = 5.0  # 50% faces
    fps_low: float = 3.0    # 20% faces
...
class FaceScheduler:
    ...
    def _compute_priority_scores(
        ...
        for track_id in track_ids:
            ...
            # Time decay (older checks get boosted)
            time_since_last_check = current_ts - track_state.last_processed_ts
            
            # Ensure minimum interval before re-checking same track
            if time_since_last_check < self.config.min_check_interval_sec:
                time_decay = 0.0  # Penalize recently-checked tracks
            else:
                time_decay = min(
                    100.0,
    ...
    def _select_top_k(self, scores: Dict, k: int) -> List[int]:
        ...
        """
        # Filter scores (remove metadata counts)
        track_scores = {
    ...
    def _record_metrics(
        ...
        try:
            ...
            # Deep Efficiency Logic: Record directly to the central metrics collector
            ...
            # 'starved' is not explicitly tracked here yet, can be added later if priority forces it
