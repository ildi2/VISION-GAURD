...
def run() -> None:
    ...
    if identity_mode_requested == "multiview" and multiview_available:
        identity_primary = IdentityEngineMultiView(governance_cfg=cfg.governance)
    ...
    if hasattr(cfg, "vision_identity"):
        try:
            vision_mode = getattr(cfg.vision_identity, "mode", "classic")
            if isinstance(vision_mode, str):
                vision_mode = vision_identity_mode.strip().lower()
            
            if vision_mode in ("continuity", "shadow_continuity"):
                from identity.continuity_binder import ContinuityBinder
                
                continuity_binder = ContinuityBinder(cfg.vision_identity)
                continuity_mode = vision_identity_mode
                ...
                shadow_status = "shadow mode (observe-only)" if vision_mode == "shadow_continuity" else "real mode (GPS carry)"
                ...
                log.info(
                    ...
                    continuity_binder.max_bbox_displacement_frac
                )
        except Exception:
            log.exception(
        ...
        log.debug("Vision Identity continuity mode disabled (no vision_identity config section)")
    ...
    if hasattr(cfg, "privacy") and getattr(cfg.privacy, "enabled", False):
        if PrivacyPipeline is not None:
            try:
                privacy_pipeline = PrivacyPipeline(cfg.privacy)
                privacy_enabled = True
                log.info(
                    "Privacy pipeline initialized | audit=%s | preview=%s",
                    privacy_pipeline.audit_file_path,
                    getattr(cfg.privacy.ui, "show_privacy_preview", True),
                )
            except Exception:
                log.exception(
                    "Failed to initialize privacy pipeline; continuing without privacy"
                )
                privacy_pipeline = None
        else:
            log.warning(
                "Privacy enabled in config but PrivacyPipeline module not available"
            )
    else:
        log.debug("Privacy pipeline disabled (privacy.enabled=false or missing)")
    ...
    try:
        while True:
            ...
            if privacy_pipeline is not None:
                try:
                    privacy_pipeline.ingest(frame, tracks, decisions)
                except Exception:
                    log.exception("Privacy pipeline ingest failed; continuing without privacy")
            ...
            if privacy_pipeline is not None and privacy_enabled:
                try:
                    privacy_ui_cfg = getattr(cfg.privacy, "ui", None)
                    show_preview = getattr(privacy_ui_cfg, "show_privacy_preview", True) if privacy_ui_cfg else True
                    
                    if show_preview:
                        privacy_frame = privacy_pipeline.get_latest_privacy_frame()
                        preview_title = getattr(privacy_ui_cfg, "preview_window_title", "Privacy Output (Delayed)") if privacy_ui_cfg else "Privacy Output (Delayed)"
                        
                        if privacy_frame is not None:
                            cv2.imshow(preview_title, privacy_frame)
                        else:
                            placeholder = np.zeros((120, 320, 3), dtype=np.uint8)
                            cv2.putText(
                                placeholder, "Buffering...", (80, 70),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2
                            )
                            cv2.imshow(preview_title, placeholder)
                except Exception:
                    log.exception("Privacy preview display failed")
        ...
        if privacy_pipeline is not None:
            try:
                privacy_pipeline.shutdown()
            except Exception:
                log.exception("Privacy pipeline shutdown failed")
