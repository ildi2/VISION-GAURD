...
# --- MOCKS ---
# Replicate schemas/tracklet.py minimal
@dataclass
class Tracklet:
    track_id: int
    camera_id: str
    ...
    confidence: float
    # Extras
    score: float = 0.9  # main_loop expects .score usually on the wrapper, but let's assume it's there
    bbox: Tuple[float, float, float, float] = (0,0,10,10)

# Import real MergeManager if possible, otherwise mock context
try:
...
def test_tracklet_attribute_access():
    ...
    try:
        # Verify .id is gone/fails (as we want to use track_id)
        _ = t.id
...
def test_merge_manager_adapter():
    ...
    # Simulate main_loop call
    # merge_manager.update_track_state(track_id, track.bbox, track.score, current_ts)
    try:
