...
def test_config_loading():
    ...
    try:
        ...
        if hasattr(cfg, "governance") and hasattr(cfg.governance, "scheduler"):
            ...
            # Get key properties from SchedulerConfig dataclass
...
def test_priority_scoring():
    ...
    try:
        ...
        # Initialize track states (normally done in compute_schedule)
        current_ts = 1000.0
        for track_id in track_ids:
            if track_id not in scheduler.track_states:
                ...
                scheduler.track_states[track_id] = TrackScheduleState(
                    track_id=track_id,
                    last_processed_ts=current_ts - 10.0  # All checked 10s ago
                )
        
        scores = scheduler._compute_priority_scores(track_ids, binding_states, current_ts)
        
        # Verify priority order (with debugging)
        # PENDING=80, UNKNOWN=50, CONFIRMED_WEAK=20, CONFIRMED_STRONG=10
        # Plus time decay of ~10s * 20 = 200 for each
        try:
...
def test_minimum_interval_enforcement():
    ...
    try:
        ...
        # Frame 2 (very close): Some should be skipped
        ...
        # Frame 3 (after interval): Should reschedule
