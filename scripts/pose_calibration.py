...
# A single pose sample: (yaw_deg, pitch_deg, quality)
...
def _build_insight_app(cfg: FaceConfig) -> FaceAnalysis:
    ...
    # Decide on ctx_id from cfg.device.device
    ...
    # Important: use same det_size as in your logs.
...
def _compute_quality(
    ...
    # 1) Base from detection confidence (clamped 0..1)
    ...
    # 2) Pose penalty: 1.0 when within max_yaw/pitch, dropping towards 0
...
def main():
    ...
    while True:
        ...
        if largest is not None:
            ...
            # InsightFace pose convention: typically (pitch, yaw, roll) in degrees.
            # We map:
            #   yaw_deg   = pose[1]
            #   pitch_deg = pose[0]
            yaw_deg = 0.0
        ...
        # If we have a face, we can record samples for the pressed key.
        # Recompute yaw/pitch/q for the recorded sample (same as above).
