# scripts/mv_report.py
#
# CLI tool to inspect the pseudo-3D / multiview head models
# derived from the encrypted face gallery.
#
# It is read-only and does NOT modify the gallery.
#
...
def _print_person_report(
    ...
    *,
    ...
    # Print primary bins in fixed order, then any extra (OCCLUDED/UNKNOWN) that exist.
    printed_bins = set()
    ...
    # Non-primary but present bins (e.g. OCCLUDED / UNKNOWN)
    extra_bins = sorted(
...
def _print_global_summary(models: Dict[str, MultiViewPersonModel]) -> None:
    ...
    # Small histogram of "how many bins per person"
    hist = {}
    for m in models.values():
...
def main(argv: Optional[list[str]] = None) -> None:
    # Simple console logging, we don't need the full core/logging_setup here.
    logging.basicConfig(
        level=logging.INFO,
    ...
    # ------------------------------------------------------------------ #
    # Build the same FaceConfig / FaceGallery as runtime uses           #
    # ------------------------------------------------------------------ #
    ...
    face_cfg = default_face_config(
        prefer_gpu=True,
        base_dir=base_dir,
        face_section=None,  # let defaults handle thresholds/route/etc.
    )
    ...
    # If gallery is locked due to crypto error, FaceGallery will have
    # exposed this via its logging + locked flag. Here we just proceed;
    # if there are no persons/models, summary will say so.
    ...
    # If a person_id is given, print its report (if present).
    if args.person_id:
        ...
        # Also show a small global context underneath, so you see where
        # this person sits relative to others.
        _print_global_summary(models)
        return

    # No person_id → just global summary.
    _print_global_summary(models)
