...
from vision_identity.fusion_engine import VisionFusionEngine
from vision_identity.bindings import (
    BindingManager,
    GaitTemplate,
    FaceGaitBinding,
    BindingStrength,
...
from vision_identity.config import default_vision_identity_config
...
def test_binding_creation():
    ...
    engine = VisionFusionEngine(default_vision_identity_config())
    ...
    from vision_identity.types import VisionDecision
    ...
    print(f"  Decision confidence: {decision.vision_identity_confidence:.2f}")
...
def test_binding_confidence_boost():
    ...
    assert boost_amount <= 0.155, "Boost exceeded max (with epsilon)"
...
def test_end_to_end_workflow():
    ...
    if should_bind:
        ...
        binding = engine.binding_manager.create_binding(
            ...
            face_confidence=decision1.vision_identity_confidence,
        )
    ...
    decision2 = VisionDecision(
        track_id=track_id,
        final_identity="alice",
        vision_confidence=0.70,
        state=VisionState.TENTATIVE,
        decision_reason=VisionReason.GAIT_TENTATIVE_NO_FACE_ANCHOR,
    ...
    if binding:
        ...
        boost_amount = boosted - decision2.vision_identity_confidence
        print(f"  ✓ Binding boost applied: {decision2.vision_identity_confidence:.2f} → {boosted:.2f} (+{boost_amount:.3f})")
