# identity/multiview_matcher.py
#
# Pose-aware matcher on top of the pseudo-3D / multi-view head database.
#
# Responsibilities:
#   - Take a single live face sample (embedding + yaw/pitch + quality).
#   - Decide which pose bin is most appropriate for this sample.
#   - Search the corresponding bin’s centroids (and optionally a global
#     fallback index) for the closest person.
#   - Map distance/score to a qualitative strength: strong / weak / none.
#
# This module does NOT:
#   - Handle tracking or temporal smoothing (that’s the identity engine).
#   - Modify the encrypted FaceGallery on disk.
#   - Change MultiViewPersonModel objects (they are read-only here).
#
# It is a pure matching layer between:
#   face.multiview_gallery_view.MultiViewGalleryView  (pseudo-3D heads)
#   and
#   identity.identity_engine_multiview                 (temporal logic).
...
try:  # pragma: no cover - import-time decision
...
class MultiViewMatcher:
    ...
    def __init__(
        ...
        # Per-bin indices: bin_name -> _BinIndex
        self._bin_indices: Dict[str, _BinIndex] = {}

        # Global index (all centroids in one place) for fallback searches.
        self._global_index: _BinIndex = _BinIndex()

        # Dirty flag: set when the underlying gallery changes.
        self._dirty: bool = True

        # Metric: for now we assume cosine on L2-normalised vectors.
        # If you later extend MultiViewConfig to support "l2", you can
        # route that here.
        self._metric: str = "cosine"

        # Cache important config values with safe defaults so older configs
        # (without multiview fields) do not break.
        self._dim: int = int(getattr(self._cfg, "dim", 512))

        # Quality threshold: support both min_quality_for_match and min_quality.
        ...
        # Distance thresholds: support both strong_distance/weak_distance
        # and strong_dist/weak_dist, mirroring IdentityEngineMultiView.
    ...
    def _ensure_indices(self) -> None:
        ...
        # Ensure multi-view models are up-to-date.
        ...
        # Collect centroids from each MultiViewPersonModel.
        for pid, model in models.items():
            for bin_name, bin_state in model.bins.items():
                # NOTE: num_samples is a *method* on bin_state, not a field.
                try:
                    num = int(bin_state.num_samples())
                except TypeError:
                    # Extreme defensive fallback; if API changes we skip safely.
                    logger.warning(
                ...
                # Dimension sanity-check: must match matcher dim.
    ...
    def match(
        ...
        *,
        ...
        # If there are no centroids at all, we can only return "none".
        ...
        # Reject very low-quality observations early.
        ...
        # Validate & normalise query embedding.
        ...
        # Decide primary pose bin for this observation.
        if hasattr(self._cfg, "choose_bin"):
            primary_bin = str(self._cfg.choose_bin(yaw=yaw, pitch=pitch))
        else:
            # Very conservative fallback if config is old.
            primary_bin = "FRONT"

        considered_bins: List[str] = [primary_bin]

        # Try primary bin first; if empty, fall back to global.
        ...
        # Map distance / score to strength.
        strength = self._classify_strength(best.distance, best.score)

        if strength == "none":
            # Distances exist but are not good enough.
            return MultiViewMatchResult(
                strength="none",
                best=None,
