...
# Wave 2.1 / 2.2 – hardened load/save behaviour, clearer crypto failure handling,
# multi-condition templates, and clean CRUD hooks.
#
# Wave 3.0 – pseudo-3D / multi-view support (derived from templates):
#   - Build a MultiViewPersonModel per person from their templates
#     using pose bins and centroids (front / left / right / up / down / ...).
#   - Keep multi-view models in-memory only (NOT persisted); they are
#     derived from the stored templates via face.multiview_builder.
#   - Existing FAISS search and gallery file format remain unchanged.
#
# Wave 3.1 – watchlist-ready metadata:
#   - PersonEntry.metadata is the canonical place for:
#         * 'source'       : "LIVE_ENROLLMENT" | "IMAGE_ONLY" | "HYBRID"
#         * 'country'      : ISO-ish country tag (e.g. "IT", "UK")
#         * 'notes'        : free-text description
#         * any admin fields (case id, etc.)
#   - PersonEntry.created_at gives a stable enrollment timestamp.
#   - FaceTemplate.metadata remains the place for per-template fields:
#         * pose / quality (yaw_deg, pitch_deg, quality, pose_bin_hint, timestamp)
#         * 'source_type'  : "LIVE_CAPTURE" | "EXTERNAL_IMAGE"
#         * 'origin_file'  : original filename/path (for diagnostics)
#         * 'quality_level': "HIGH" | "MEDIUM" | "LOW"
#
#   These additions are fully backward-compatible with existing gallery files.
...
# Pseudo-3D / multi-view types and builder
...
class FaceGallery:
    ...
    def __init__(
        ...
        # Multi-view configuration & builder (pseudo-3D layer)
        ...
        # Gallery path (may be None in tests, but in normal use it's a file)
    ...
    def enroll_person(
        ...
        *,
        # per-template attributes for vector-only usage:
        conditions: Optional[Sequence[str]] = None,
        # high-level person metadata:
        category: Category = "resident",
        name: Optional[str] = None,
        metadata: Optional[Dict[str, Any]] = None,
        # optional explicit ID (for guided-enroll / controlled IDs):
        person_id: Optional[str] = None,
        surname: Optional[str] = None,
        # ---- backward-compat keywords used by classic enrollment_cli ----
        ...
        """
        # ------------------------------------------------------------------
        # Backward-compatibility shim: embeddings -> templates
        # ------------------------------------------------------------------
        if templates is None and embeddings is not None:
            ...
            # Attach notes into person-level metadata if provided.
        ...
        # ------------------------------------------------------------------
        # Validate presence of templates
        # ------------------------------------------------------------------
        if not templates:
            raise ValueError("enroll_person requires at least one template embedding.")

        # ------------------------------------------------------------------
        # Decide / allocate person_id
        # ------------------------------------------------------------------
        if person_id is None:
            # Default allocation uses the internal numeric sequence and the
            # 'p_' prefix. Watchlist-specific prefixes (e.g. 'p_w_') can be
            # allocated by higher-level tools that pass an explicit person_id.
            person_id = self._allocate_person_id()
        else:
            # If caller supplies an explicit ID that already exists, we log a
            # warning and continue by appending templates to that person. This
            # is safer than silently overwriting.
        ...
        # ------------------------------------------------------------------
        # Prepare person-level metadata (including optional surname)
        # ------------------------------------------------------------------
        person_meta = dict(metadata or {})
        if surname:
            # Keep surname structured without changing existing schema.
            person_meta.setdefault("surname", surname)

        # Either create or reuse PersonEntry.
        if person_id in self._persons:
            entry = self._persons[person_id]
            # Optionally update category/name/metadata if freshly provided.
            if category is not None:
                entry.category = category
            if name is not None:
                entry.name = name
            entry.metadata.update(person_meta)
            # We intentionally do NOT touch created_at when appending
            # templates to an existing person.
        else:
            entry = PersonEntry(
                person_id=person_id,
                category=category,
                ...
                # created_at default_factory gives now()
            )
            self._persons[person_id] = entry

        # ------------------------------------------------------------------
        # Normalise per-template data
        # ------------------------------------------------------------------
        # Case A: templates are FaceTemplate objects
        if isinstance(templates[0], FaceTemplate):
            for tmpl in templates:  # type: ignore[union-attr]
                # Ensure correct embedding type & dim
                tmpl.embedding = self._validate_embedding(tmpl.embedding)
                # Normalise metadata keys for multi-view (yaw_deg, pose_bin_hint, ...)
                self._normalise_template_metadata(tmpl)
                entry.templates.append(tmpl)

        # Case B: templates are raw embeddings (np.ndarray-like)
        else:
            ...
            for emb_vec, cond in zip(templates, conditions):  # type: ignore[assignment]
        ...
        # Mark both classic index and multi-view models as dirty.
    ...
    def add_template(
        ...
        embedding: np.ndarray,
        *,
        # note: star is causing syntax highlight confusion; it's fine syntactically
        ...
        # Normalise any pose-related metadata if present.
        self._normalise_template_metadata(tmpl)
    ...
    def delete_person(self, person_id: str) -> bool:
        ...
        if person_id in self._persons:
            ...
            self._index_dirty = True
            self._mv_dirty = True
            # If we had a multi-view model for this person, drop it as well.
    ...
    def _ensure_multiview_models(self) -> None:
        ...
        # Build models for all persons using MultiViewBuilder.
        for pid, entry in self._persons.items():
            if not entry.templates:
                continue

            # Convert templates -> MultiViewSample list using builder helper.
    ...
    def load_if_exists(self, path: Optional[Path] = None) -> None:
        ...
        try:
            self.load(path)
            # Successful load → clear any previous error/lock.
            ...
            # Any other crypto-related error.
            self._locked_due_to_load_error = True
            self._last_load_error = str(exc)
            logger.error(
            ...
            # Non-crypto errors (IO, JSON etc.) – still log but do not lock,
            # because they might be transient (permissions, etc.).
            self._last_load_error = str(exc)
            logger.error(
            ...
            persons_data.append(
                {
                    ...
                    "metadata": p.metadata,
                    "templates": t_list,
                    # New in Wave 3.1: person-level created_at.
    ...
    def _from_serializable(self, data: Dict[str, Any]) -> None:
        ...
        for p_data in persons_data:
            ...
            for t_data in t_list:
                ...
                # Also normalise any legacy pose metadata on load.
                self._normalise_template_metadata(tmpl)
                entry.templates.append(tmpl)
