# identity/binding.py
# ============================================================================
# PHASE C: BINDING STATE MACHINE - IDENTITY STABILITY & MARGIN ENFORCEMENT
# ============================================================================
#
# Purpose:
#   Convert noisy per-frame identity evidence into stable, margin-enforced
#   identity decisions. Implements state machine with evidence accumulation,
#   contradiction detection, and controlled switching.
#
# Key Innovation:
#   State-aware binding prevents false positives through:
#   1. Evidence accumulation (not single-frame decisions)
#   2. Margin enforcement (best > second_best + threshold)
#   3. Contradiction detection (anti-lock-in mechanism)
#   4. Gradual confidence increase (UNKNOWN → PENDING → CONFIRMED)
#
# Responsibilities:
#   1. Maintain per-track binding state
#   2. Accumulate face evidence into buffers
#   3. Apply state transition rules
#   4. Enforce margin requirements
#   5. Detect contradictions
#   6. Emit structured decisions with reasons
#   7. Record metrics for diagnostics
#   8. Never crash pipeline (exception safety)
...
class EvidenceRecord:
    ...
    score: float                    # Gallery match score (0-1)
    second_best_score: float       # Next best score
    margin: float                  # score - second_best_score
    quality: float                 # Face quality from Phase B
    timestamp: float               # When this evidence arrived
...
class BindingDecision:
    ...
    track_id: int
    person_id: Optional[str]       # Bound person or None
    binding_state: str              # Current state name
    confidence: float              # 0-1 confidence in binding
    reason: str                    # Why this decision
    margin: float = 0.0            # Last margin value
    state_changed: bool = False    # Did state transition occur
...
class TrackBindingState:
    ...
    pending_person_id: Optional[str] = None  # For SWITCH_PENDING
...
class BindingManager:
    ...
    def __init__(
        ...
        try:
            ...
            # Case 1: Full system config (cfg.governance.binding)
            if cfg and hasattr(cfg, 'governance') and hasattr(cfg.governance, 'binding'):
                binding_config = cfg.governance.binding
            
            # Case 2: Direct binding config (passed from IdentityEngine)
            # Detect by checking for characteristic fields and ABSENCE of 'governance'
            elif cfg and hasattr(cfg, 'confirmation') and hasattr(cfg, 'switching'):
                 binding_config = cfg
            
            # Case 3: Direct binding config (duck typing for basic enabled check)
            ...
            if binding_config:
                ...
                # No valid config found - Default to ENABLED for robust fallback
    ...
    # ========================================================================
    # PRIVATE METHODS: STATE MACHINE LOGIC
    # ========================================================================
    ...
    def _apply_state_transitions(
        ...
        try:
            ...
            # Check contradiction counter (can downgrade any state)
    ...
    def _transition_from_unknown(
        ...
        try:
            ...
            # ════════════════════════════════════════════════════════════════════
            # ▼▼▼ ORIGINAL TRANSITION LOGIC (CURRENTLY ACTIVE) ▼▼▼
            # 
            # TO SWITCH TO OPTION C:
            # ─────────────────────────
            # Step 1: Add ''' on the line AFTER this comment block (before 'strong_samples')
            # Step 2: Add ''' after line ~423 (after the last logger.info)
            # Step 3: Remove the ''' from lines ~443 and ~517 (Option C block)
            # ════════════════════════════════════════════════════════════════════
            
            # Count strong matches (high score + good margin)
            # Count strong matches using Adaptive Strength Logic
            # Tier 1: High Score (Classic) - Score >= 0.60, Margin >= 0.05
            # Tier 2: High Margin (Robust) - Score >= 0.50, Margin >= 0.20
            strong_samples = []
            for e in buffer:
            ...
            if len(strong_samples) >= min_samples:
                ...
                if len(recent_strong) >= min_samples:
                    # All recent samples same person?
            ...
            # ════════════════════════════════════════════════════════════════════
            # ▲▲▲ END ORIGINAL TRANSITION LOGIC ▲▲▲
            # ════════════════════════════════════════════════════════════════════
            
            
            # ════════════════════════════════════════════════════════════════════
            # ▼▼▼ OPTION C: CLOSE-UP ADAPTIVE MODE ▼▼▼
            # 
            # TO ACTIVATE OPTION C:
            # ─────────────────────────
            # Step 1: Disable ORIGINAL above by adding ''' before and after its block
            # Step 2: Remove the ''' on lines ~443 and ~517 below
            #
            # EFFECT: Auto-detects close-up faces and applies relaxed thresholds
            # USE FOR: Mixed environments where users approach camera closely
            # HOW IT WORKS:
            #   - Detects if face quality is high (>0.60) which indicates close-up
            #   - For close-up: uses min_samples=2, min_score=0.55, min_margin=0.03
            #   - For normal: uses original thresholds from _default_confirmation_config
            # ════════════════════════════════════════════════════════════════════
            '''
    ...
    def _transition_from_pending(
        ...
        try:
            ...
            # Recent samples matching current person
            # Recent samples matching current person using Adaptive Strength
            recent_matches = []
            for e in buffer:
                ...
                if (e.score >= self._get_threshold('confirmation.min_avg_score', 0.60)):
                    is_valid = True
                # Tier 2: Weak match but high margin (Adaptive)
                elif (e.score >= 0.50 and e.margin >= 0.20):
                    is_valid = True
                    
                # We do not strictly enforce min_avg_score here, we filter for "valid evidence"
                # The avg_score check later determines if we transition.
                if is_valid:
    ...
    def _transition_from_switch_pending(
        ...
        try:
            ...
            # Recent samples of pending person
            pending_matches = [
            ...
            # Check if alternative sustained
            if len(pending_matches) >= min_samples:
                ...
                track_state.state_enter_ts = current_ts
                track_state.contradiction_counter = 0  # Reset on successful switch
                logger.debug(
                    f"Track {track_state.track_id} switched to {track_state.person_id}"
                )
            else:
                # Timeout or fade away → fall back
    ...
    def _check_contradiction_downgrade(
        ...
        try:
            ...
            # Check for contradictions (current person gets low score, alt gets high)
    ...
    #
    # ╔═══════════════════════════════════════════════════════════════════════════╗
    # ║         THRESHOLD OPTIONS - EASY TOGGLE SYSTEM                            ║
    # ╠═══════════════════════════════════════════════════════════════════════════╣
    # ║                                                                           ║
    # ║  THREE MODES AVAILABLE:                                                   ║
    # ║  ─────────────────────                                                    ║
    # ║  • ORIGINAL: Production thresholds (ACTIVE by default)                   ║
    # ║  • OPTION B: Lower thresholds for 30-50% faster recognition              ║
    # ║  • OPTION C: Close-up adaptive (see _transition_from_unknown method)     ║
    # ║                                                                           ║
    # ║  HOW TO SWITCH TO OPTION B:                                               ║
    # ║  ───────────────────────────                                              ║
    # ║  Step 1: Add ''' before line 928 (before 'class ConfirmationConfig')     ║
    # ║  Step 2: Add ''' after line 933 (after 'return ConfirmationConfig()')    ║
    # ║  Step 3: Remove ''' from lines 950 and 959                               ║
    # ║                                                                           ║
    # ║  HOW TO SWITCH TO OPTION C:                                               ║
    # ║  ───────────────────────────                                              ║
    # ║  Option C is in _transition_from_unknown() method (around line 360)      ║
    # ║  See instructions there - it uses the thresholds defined here.           ║
    # ║                                                                           ║
    # ╚═══════════════════════════════════════════════════════════════════════════╝
    #
    ...
    def _default_confirmation_config(self) -> Any:
        ...
        # ════════════════════════════════════════════════════════════════════
        # ▼▼▼ ORIGINAL THRESHOLDS (CURRENTLY ACTIVE) ▼▼▼
        # To disable: Add ''' on the line BEFORE 'class' and AFTER 'return'
        # ════════════════════════════════════════════════════════════════════
        #unknown_min_quality: 0.68 → 0.58 check unkwon face acceptance  , changed from 0.68 to 0.58
        class ConfirmationConfig:
            min_samples_strong = 3       # Require 3 strong samples
            min_samples_weak = 5         # Require 5 weak samples as alternative
            window_seconds = 3.0         # Evidence window 
            min_avg_score = 0.75         # Average score threshold
            min_avg_margin = 0.08        # Margin over second-best
            min_quality_for_strong = 0.60  # Quality gate for strong samples
        return ConfirmationConfig()
        # ════════════════════════════════════════════════════════════════════
        # ▲▲▲ END ORIGINAL THRESHOLDS ▲▲▲
        # ════════════════════════════════════════════════════════════════════
        
        
        # ════════════════════════════════════════════════════════════════════
        # ▼▼▼ OPTION B: LOWER THRESHOLDS (FASTER RECOGNITION) ▼▼▼
        # 
        # TO ACTIVATE OPTION B:
        # ──────────────────────
        # 1. Disable ORIGINAL above by adding ''' before and after its block
        # 2. Remove the ''' on lines 958 and 967 below
        #
        # EFFECT: 30-50% faster recognition, slightly higher false positive risk
        # USE FOR: Indoor environments, controlled lighting, trusted users
        # ════════════════════════════════════════════════════════════════════
        '''
        class ConfirmationConfig:
