...
# Tiny face size below which embeddings are mostly noise (in pixels^2)
MIN_FACE_AREA_HARD_REJECT = 20 * 20
# Soft warning threshold: small but still usable
MIN_FACE_AREA_WARN = 40 * 40

# ---------------------------------------------------------------------------
# Dataclasses for watchlist enrollment
# ---------------------------------------------------------------------------
...
class FaceTemplateProposal:
    ...
    pitch_deg: float
    roll_deg: float
    pose_bin: str  # FRONT / LEFT / RIGHT / UP / DOWN / UNKNOWN
...
# ---------------------------------------------------------------------------
# Helpers: allocation, quality, pose bins, extraction from FaceCandidate
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Public API – person draft + image processing + commit
# ---------------------------------------------------------------------------
...
def process_image(
    ...
    *,
    ...
    # 3) Detect faces with same backend as runtime / enrollment
    ...
    # Multi-face case without explicit index → return summaries
    ...
    # Single chosen detection (either 1 face or user-selected index)
    ...
    # 5) Embedding (using detector's embedding + passthrough embedder)
    aligned_bgr, embedding = _extract_aligned_and_embedding(det, embedder)
    # We don't strictly need aligned_bgr for quality here; quality uses full frame + bbox.

    # 6) Quality estimation.
    # IMPORTANT: In your project compute_full_quality RETURNS A FLOAT (q_overall),
    # as seen in classic enrollment:
    #    q = compute_full_quality(image=frame, bbox=cand.bbox, ...)
    #
    # We mirror that pattern and treat the result as scalar.
    try:
        ...
        # Older signature without cfg or slightly different params
        try:
            q_val = compute_full_quality(
                image=img_bgr,
                bbox=bbox_xyxy,
...
def commit_watchlist_enrollment(
    ...
    *,
    ...
    if copy_images:
        ...
        storage_root = None
        person_dir = None  # type: ignore[assignment]
    ...
    for idx, prop in enumerate(proposals):
        ...
        if copy_images and storage_root is not None:
            ...
            dest_path = person_dir / dest_name  # type: ignore[operator]
            try:
