...
class MultiViewBuilder:
    ...
    def build_person_model(
        ...
        *,
        ...
        # Filter by quality and re-bin using the current config.
        filtered: List[MultiViewSample] = []
        for s in samples_list:
            if float(s.quality) < self.cfg.min_quality_for_model:
                continue

            # If pose_bin is UNKNOWN/OCCLUDED or thresholds changed, re-classify
            # using yaw/pitch + current config. (Keeps model consistent.)
    ...
    # ------------------------------------------------------------------
    # Incremental / EMA-style update hooks (Layer 2)
    # ------------------------------------------------------------------
    ...
    def update_person_model_ema(
        ...
        # Normalise new samples and re-bin according to current config.
        for s in new_samples:
            ...
            # Update average quality as a simple running mean.
            # (For simplicity we recompute from samples; small N per bin.)
            _, avg_q = _compute_centroid(bin_obj.samples)
            bin_obj.avg_quality = avg_q

        # Update global timestamp based on latest sample across all bins.
    ...
    # ------------------------------------------------------------------
    # Helpers for building samples from "template-like" objects
    # ------------------------------------------------------------------
    ...
    def samples_from_templates(
        ...
        *,
        # Preferred keys written by guided enrollment / modern code:
        yaw_key: str = "yaw_deg",
        pitch_key: str = "pitch_deg",
        roll_key: str = "roll_deg",
        quality_key: str = "quality",
        # Backwards-compatible fallbacks are handled inside.
        ...
        for t in templates:
            ...
            # Extract metadata as a dict, but keep it unchanged so we don't
            # break other consumers that also use template.metadata.
            meta: Dict[str, Any] = {}
            raw_meta = getattr(t, "metadata", None)
            if isinstance(raw_meta, Mapping):
                meta = dict(raw_meta)

            # --- Pose extraction with fallbacks (for older galleries) ---
            ...
            # --- Quality extraction with fallbacks ---
            quality_val = meta.get(quality_key, default_quality)
            # legacy key: "face_quality"
