...
class PoseBin(str, Enum):
    ...
    # Not a guided view, but useful for completeness:
...
def posebin_from_hint(hint: Any) -> PoseBin:
    ...
    # Try matching by value ("front", "left", etc.)
    for b in PoseBin:
        if low == b.value:
            return b

    # Try matching by name ("FRONT", "LEFT", etc.)
    up = s.upper()
    for b in PoseBin:
        if up == b.name:
            return b
...
class MultiViewConfig:
    ...
    # FRONT:
    #   |yaw|   <= front_yaw_max_deg
    #   |pitch| <= front_pitch_max_deg
    front_yaw_max_deg: float = 15.0
    front_pitch_max_deg: float = 12.0  # slightly wider than 10°, more tolerant

    # SIDE views:
    #   side_yaw_min_deg <= |yaw| <= side_yaw_max_deg
    #   sign(yaw) decides LEFT (<0) vs RIGHT (>0)
    #
    # Your calibration on this webcam:
    #   |yaw| ≈ 60–70° when really looking left/right.
    side_yaw_min_deg: float = 20.0
    side_yaw_max_deg: float = 70.0

    # UP:
    #   pitch >= up_pitch_min_deg
    # DOWN:
    #   pitch <= -down_pitch_min_deg
    #
    # From pose_calibration:
    #   UP   ≈ +18–25°
    #   DOWN ≈ -24 to -40°
    ...
    # Hard cap on number of samples we keep per bin per person.
    max_samples_per_bin: int = 16

    # Minimum quality required for a sample to be used when building
    # the multi-view model (centroids, stats). This is separate from
    # runtime quality gates in FaceConfig.thresholds.
    min_quality_for_model: float = 0.40

    # Centroid update behaviour (for EMA-style learning later).
    # When adding new high-quality samples, we can update the centroid:
    #   new_centroid = (1 - ema_alpha) * old + ema_alpha * sample
...
class MultiViewSample:
    ...
    # Pose estimates in degrees (can be None if unavailable).
    yaw_deg: Optional[float] = None
    pitch_deg: Optional[float] = None
    roll_deg: Optional[float] = None

    # Quality in [0, 1] (recommended).
    quality: float = 0.0

    # Pose bin assigned according to MultiViewConfig.
    pose_bin: PoseBin = PoseBin.UNKNOWN

    # Metadata for diagnostics / provenance ("enroll_guided", "runtime", etc.)
    source: str = "unknown"
...
class MultiViewBin:
    ...
    # Centroid embedding for this bin (L2-normalised).
    centroid: Optional[np.ndarray] = None

    # Simple stats (can be recomputed when needed).
...
class MultiViewPersonModel:
    ...
    # Optional free-form notes / diagnostics
...
def classify_pose_bin(
    ...
    *,
    ...
    # If we don't have pose estimates, we cannot bin reliably.
    ...
    if cfg.side_yaw_min_deg <= ay <= cfg.side_yaw_max_deg:
        # Convention: negative yaw = subject turned left (camera sees right profile)
