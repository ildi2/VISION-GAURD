...
# Import from main system's core
from core.device import select_device

logger = logging.getLogger(__name__)

# Auto-detect gait_subsystem directory
...
class GaitThresholdConfig:
    ...
    max_match_distance: float = 0.30   # STRONG match threshold (cosine distance)
    max_weak_match_distance: float = 0.40  # WEAK match threshold
    min_gait_quality: float = 0.5      # Min average confidence for a sequence
    min_match_margin: float = 0.10     # Min separation between top-2 candidates
...
class GaitRouteConfig:
    ...
    min_sequence_length: int = 30      # Required frames for temporal analysis
...
class GaitRobustConfig:
    ...
    """
    min_seq_len: int = 30              # Min frames to enter EVALUATING
    eval_period: float = 0.7           # Seconds between evaluations (scheduling)
    
    # Quality Gates
    quality_min: float = 0.55          # Q_min: Min quality to evaluate
    quality_confirm: float = 0.65      # Q_confirm: Min quality to confirm
    
    # Decision Policy
    threshold_candidate: float = 0.60  # T_candidate: Lowered to catch 0.60 peaks
    threshold_confirm: float = 0.70    # T_confirm: Lowered to 0.70 (standard robust)
    margin_confirm: float = 0.05       # M_confirm: Slightly looser margin
    confirm_streak: int = 2            # K: Consecutive confirms required
    
    # Regime Gates
    min_motion: float = 0.05           # Min motion proxy to avoid "STILL" regime

    # Anthropometric Gating [DEEP ROBUST]
    anthro_threshold: float = 0.15     # Max allowed geometric distance (L1 diff in ratios)
    anthro_penalty_weight: float = 0.5 # How much to penalize sim if shape mismatches
...
def default_gait_config(
    ...
    # Use gait_subsystem directory as base
    base_dir = base_dir or GAIT_SUBSYSTEM_DIR
    
    # Path resolution for models
    ...
    # Path resolution for Data (Gallery)
    # User Requirement: Store data in MAIN system's data folder, not subsystem
    # base_dir is gait_subsystem/. Main data is ../data relative to it.
    ...
    return GaitConfig(
        ...
        thresholds=GaitThresholdConfig(
            max_match_distance=0.30,      # Stricter for robustness
