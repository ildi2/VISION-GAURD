...
def extract_yolo_boxes_keypoints(yolo_result) -> Tuple[List[List[float]], List[float], List[np.ndarray]]:
    ...
    try:
        ...
        if hasattr(yolo_result, "keypoints") and yolo_result.keypoints is not None:
            # We prioritize .xyn for normalized coordinates required by the gait extractor
...
class Phase1PerceptionEngine(PerceptionEngine):
    ...
    def process_frame(self, frame: Frame) -> List[Tracklet]:
        ...
        # Step 1: Object Detection and Pose Estimation
        ...
        for tr in tracks:
            ...
            # Associate current skeletons to tracks via IOU
            best_iou = 0.0
            associated_kp = None
            ...
            # Apply EMA smoothing to reduce keypoint jitter
        ...
        # Step 5: Clean up old tracks and store valid data in Ring Buffer
