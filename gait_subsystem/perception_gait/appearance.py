...
def appearance_distance(a: np.ndarray, b: np.ndarray) -> float:
    ...
    if na < 1e-6 or nb < 1e-6:
        # One of the vectors is nearly zero; treat as maximally different
        return 1.0

    cos_sim = float(np.dot(a, b) / (na * nb))  # in [-1, 1]
    return 1.0 - cos_sim  # convert to a distance-like measure
...
class AppearanceExtractor:
    ...
    def compute_feature(self, image: np.ndarray, box: Tuple[float, float, float, float]) -> Optional[np.ndarray]:
        ...
        # Optional blur to reduce high-frequency noise
        ...
        # Convert to HSV or use BGR directly
        ...
        # Compute mean color in each grid cell
        cells: List[float] = []
        for gy in range(self.config.grid_rows):
            for gx in range(self.config.grid_cols):
                ...
                mean_vals = cell.reshape(-1, 3).mean(axis=0)  # (3,)
