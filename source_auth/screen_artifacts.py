# source_auth/screen_artifacts.py
#
# Phase-4B: Screen / display artifact cue for SourceAuth.
#
# Goal:
#   Given a short time window of frames for one track, estimate how much
#   the face patch looks like it is displayed on a phone / screen rather
#   than belonging to a real 3D head in the scene.
#
# Public API:
#   compute_screen_artifact_score(frames_window, cfg) -> (score, reliable, debug)
#
# Where:
#   - frames_window: Sequence of (image_bgr, bbox_in_frame) pairs for *one* track:
#         image_bgr      : np.ndarray HxWx3, BGR, uint8 or convertible
#         bbox_in_frame  : (x1, y1, x2, y2) in full-frame coords or None
#
#   - cfg: SourceAuthConfig (or any object with the expected attributes).
#
# Returned:
#   - score    ∈ [0, 1]  (0 = no screen artifacts, 1 = strong screen indication)
#   - reliable : bool    (False if not enough evidence or window too weak)
#   - debug    : dict    (border_strength, flicker_strength, moire_strength, etc.)
#
# Heuristics implemented:
#
#   1) Border / bezel strength (RECTANGULARITY + UNIFORMITY):
#        - Crop a region around the face bbox ("outer ring").
#        - Run an edge detector.
#        - Measure:
#              * edge density along top/bottom/left/right bands,
#              * rectangularity: horizontal + vertical lines near borders
#                (Hough segments aligned with frame),
#              * bezel uniformity: low intensity variance in the border ring.
#        - Combine into border_strength ∈ [0,1].
#
#   2) Flicker strength:
#        - Build time series of mean brightness from the *face region* inside bbox.
#        - Compute relative variance and oscillation.
#        - Map to flicker_strength ∈ [0,1].
#
#   3) Moiré / high-frequency texture:
#        - For each frame, compute Laplacian variance within the face region.
#        - Aggregate across window; normalise → moire_strength ∈ [0,1].
#
#   4) Fusion:
#        - score_screen = w_border * border_strength
#                        + w_flicker * flicker_strength
#                        + w_moire  * moire_strength
#
#     with w_border typically dominant (e.g. 0.6), flicker/moiré secondary.
#
#   5) Reliability:
#        - Need enough valid frames.
#        - If border_strength is high and stable, we can be reliable even
#          with weak flicker/moiré.
#        - Otherwise we require at least some flicker or moiré signal.
#
# All steps are defensive: any error or missing data degrades to neutral
# score and reliable=False rather than breaking the pipeline.
...
FrameEntry = Tuple[np.ndarray, Optional[BBox]]  # (image_bgr, bbox_in_frame)
...
def compute_screen_artifact_score(
    ...
    # Defensive: empty or None → neutral, unreliable
    if not frames_window:
        return neutral, False, {}

    # Unpack config heuristics
    ...
    # Fusion weights (border dominant; flicker/moiré secondary).
    ...
    # Rectangularity / bezel parameters.
    ...
    # Reliability thresholds based on border temporal stability.
    ...
    for img_bgr, bbox in frames_window:
        ...
        # If face too small, not useful for screen analysis.
        ...
        if outer_region is None or outer_region.size == 0:
            # Even if no outer ring, we can still use brightness + moiré,
            # but border cue will be zero for this frame.
            outer_region = None

        try:
            # BORDER STRENGTH (edge density + rectangularity + bezel uniformity)
            ...
            # FLICKER: brightness time series will be processed later.
            ...
            # Never let a single bad frame break the cue.
            logger.exception("screen_artifacts: per-frame processing failed")
            continue

    # Reliability: need enough valid frames at all.
    ...
    # Aggregate border strength: robust mean + median.
    if border_scores:
    ...
    # Aggregate border sub-components for debug.
    ...
    #   - If border is strong AND stable → reliable even if flicker/moiré weak.
    #   - Otherwise, require some flicker/moire signal.
    border_stable = (
    ...
    if (not border_stable) and (rel_var < 0.01) and (mean_moire_value < 1e-3):
        reliable = False
        score = neutral  # degrade to neutral if essentially no signal
...
def _border_strength(
    ...
    # Canny thresholds chosen conservatively; this is heuristic.
    ...
    # Border band thickness (5% of min dimension, at least 1 pixel).
...
def _rectangularity_from_edges(
    ...
    # Parameters are heuristic and chosen to be robust and cheap.
    try:
    ...
    for line in lines:
        ...
        # Midpoint for border proximity check.
        mx = 0.5 * (x1 + x2)
        my = 0.5 * (y1 + y2)
    ...
    # Approximate perimeter of the outer_region.
...
def _bezel_uniformity_from_bands(
    ...
    # More uniform → lower std → higher score.
    raw = 1.0 - (std_val / norm)
    return _clamp01(raw)


# --------------------------------------------------------------------------- #
# Helpers: flicker strength                                                   #
# --------------------------------------------------------------------------- #
...
def _flicker_strength(
    ...
    # Normalise relative variance to [0,1] with a soft knee.
    # If rel_var == flicker_var_norm → ~0.5.
    ...
    # Oscillation pattern: look at sign changes in the first differences.
    diffs = np.diff(arr)
    if diffs.size < 2:
        osc_score = 0.0
    else:
    ...
    # Combine variance and oscillation.
...
# --------------------------------------------------------------------------- #
# Helpers: moiré / high-frequency texture                                     #
# --------------------------------------------------------------------------- #
