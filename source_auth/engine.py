# source_auth/engine.py
#
# SourceAuthEngine – source authenticity analysis
# (real 3D head vs phone / screen / printed photo).
#
# This revision integrates:
#   - motion cue (3D vs planar) from motion.py
#   - screen artifacts cue from screen_artifacts.py
#   - background consistency cue from background.py
#   - fusion module (source_auth.fusion) for clean cue → score/state mapping
#   - structured debug payloads in SourceAuthScores.debug
#   - periodic telemetry logging for easier inspection.
...
from schemas import Frame, Tracklet, IdSignals  # same style as identity engines
...
class SourceAuthEngine:
    ...
    def __init__(self, cfg: Optional[SourceAuthConfig] = None) -> None:
        # Config (fully typed object from source_auth.config)
        self.cfg: SourceAuthConfig = cfg or SourceAuthConfig()

        # Per-track identity-level state: track_id -> SourceAuthTrackState
        self._tracks: Dict[int, SourceAuthTrackState] = {}

        # Per-track time-series of landmark observations for motion analysis.
        self._landmark_history: Dict[int, Deque[LandmarkFrame]] = {}

        # Per-track time-series of frame + bbox observations for screen/bg cues.
        self._frame_history: Dict[int, Deque[FrameWindowEntry]] = {}

        # Per-track smoothed scores (for legacy/telemetry compatibility).
        self._score_history: Dict[int, float] = {}

        # Last fused scores per track (for fusion hysteresis / EMA).
        self._last_scores: Dict[int, SourceAuthScores] = {}

        # Optional nested debug buckets; kept for backwards compatibility.
        self._last_debug: Dict[int, Dict[str, Dict[str, float]]] = {}

        # Telemetry logging throttle (seconds).
    ...
    def update(
        ...
        # Pre-index IdSignals by track_id.
        ...
        for trk in tracks:
            ...
            # Phase-4E: screen + background cues.
            (
                screen_score,
                screen_reliable,
                screen_debug,
            ...
            # Keep last raw debug per subsystem (legacy bucket).
            ...
            # Phase-5.1: fusion via source_auth.fusion (clean separation).
            ...
            # Persist per-track last score for future fusion hysteresis/EMA
            # and keep legacy score_history / state in sync.
        ...
        # Housekeeping: prune stale track states and histories.
    ...
    def _maybe_update_landmark_history(
        ...
        # Quality extraction (face_quality or generic quality).
        ...
        if q_val < motion_q_min:
            # Too weak / noisy for reliable motion info.
            return

        # Bbox for this frame (prefer face_bbox_in_frame).
        bbox = getattr(sig, "face_bbox_in_frame", None)
        if bbox is None:
            bbox = getattr(sig, "bbox_in_frame", None)

        if bbox is None:
            # Fallback to Tracklet.last_box.
            try:
    ...
    # ------------------------------------------------------------------ #
    # Frame window history management (screen + background)              #
    # ------------------------------------------------------------------ #
    ...
    def _maybe_update_frame_history(
        ...
        if bbox is None:
            # Fallback to person box if IdSignals doesn’t carry face box.
            try:
                bbox = tuple(trk.last_box)
            except Exception:
                bbox = None
    ...
    def _compute_motion_cue(
        ...
        if not hist:
            # No history → neutral, unreliable, empty debug.
            return neutral, False, {}

        try:
            score, reliable, debug = compute_3d_motion_score(hist, self.cfg)
        except Exception:
            # Never let SourceAuth crash the pipeline.
            logger.exception(
        ...
        # Clamp defensively and normalise debug to floats.
        score = max(0.0, min(1.0, float(score)))
        return score, bool(reliable), {k: float(v) for k, v in debug.items()}

    # ------------------------------------------------------------------ #
    # Screen + background cue consumption                                #
    # ------------------------------------------------------------------ #
    ...
    def _compute_screen_and_background_cues(
        ...
        # --- Background cue (consistency score: 1 = same-world, 0 = mismatch) ---
        try:
    ...
    # ------------------------------------------------------------------ #
    # Fusion + temporal scoring (motion + screen + background)           #
    # ------------------------------------------------------------------ #
    ...
    def _build_scores_with_motion_screen_background(
        ...
        # Per-component effect scores (already in [0,1]).
        ...
        reliability = SourceAuthReliabilityFlags(
            ...
            # If motion is reliable, landmarks were good enough.
            enough_landmarks=motion_reliable,
            # Environment-level evidence: either screen or background cue.
            enough_background=(screen_reliable or background_reliable),
        )

        # Previous fused scores for this track (if any) for hysteresis/EMA.
        prev_scores = self._last_scores.get(track_id)

        # Delegate fusion to dedicated module.
        ...
        # Attach engine-level debug (frame counts, etc.).
        ...
        # Top-level debug (no prefix).
        scores.update_debug("", base_debug)
        # Per-subsystem debug with prefixes – keeps keys like motion_parallax_ratio, etc.
    ...
    # ------------------------------------------------------------------ #
    # Legacy temporal smoothing + state machine (no longer used in fusion)
    ...
    def _apply_temporal_smoothing_and_state(
        ...
        # EMA coefficient (0 = fully history, 1 = fully new).
        ...
        # Thresholds for discrete states.
        ...
        # Update counters for hysteresis.
        ...
        # State machine (same overall behaviour as before).
    ...
    # ------------------------------------------------------------------ #
    # Legacy neutral builder (kept for future fallbacks if needed)       #
    # ------------------------------------------------------------------ #
    ...
    # ------------------------------------------------------------------ #
    # Telemetry logging (no behaviour impact on scores)                  #
    # ------------------------------------------------------------------ #
