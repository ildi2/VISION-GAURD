# source_auth/background_consistency.py
#
# Background consistency cue for SourceAuth:
#   - Are the "background" pixels just around the face/head similar to the
#     background right outside that region?
#
# Intuition:
#   - A real 3D head in a real scene:
#       * inner band (just around head) and outer band (around head bbox)
#         share similar global background statistics (color, texture).
#   - A spoof on a phone / screen / print:
#       * inner band may be dominated by screen bezel / indoor background,
#         outer band may be camera's real background → different.
#
# Public API:
#
#   compute_background_inconsistency_score(frames_window, cfg)
#
#   where:
#       frames_window: List[Tuple[np.ndarray, Optional[(x1,y1,x2,y2)]]]
#         - image: BGR uint8 (or convertible)
#         - bbox : full-frame bbox around head / phone region (if known)
#
#       cfg: SourceAuthConfig-like object with optional fields:
#         - background_inner_margin_ratio (default 0.15)
#         - background_outer_margin_ratio (default 0.40)
#         - background_min_region_pixels   (default 500)
#         - background_hist_bins           (default 32)
#         - background_hist_color_weight   (default 0.7)
#         - background_hist_texture_weight (default 0.3)
#         - background_dist_low            (default 0.10)
#         - background_dist_high           (default 0.35)
#         - background_min_valid_frames    (default 3)
#
# Returns:
#   score   ∈ [0,1]  (0 = consistent background, 1 = strong inconsistency)
#   reliable: bool   (True only if we had enough usable frames / regions)
#   debug   : dict   (aggregated diagnostics)
#
# The design is intentionally conservative:
#   - If frames_window is empty or bboxes are missing / too small:
#         → neutral score, reliable = False.
#   - If something goes wrong, we log and fail soft with neutral / False.
...
from .types import BBoxTuple  # reuse the same alias to stay consistent
...
def compute_background_inconsistency_score(
    ...
    # Config parameters (all with safe defaults).
    ...
    for idx, (img, bbox) in enumerate(frames_window):
        ...
        # If bbox is missing, we could fall back to central region, but to
        # keep semantics clear we simply skip this frame.
        if bbox is None:
            continue

        x1, y1, x2, y2 = bbox
        # Normalize / clamp bbox to image bounds.
        ...
        if box_w * box_h < min_region_pixels:
            # Box too small to define meaningful inner/outer background.
            continue

        # Compute inner/outer regions around bbox.
        ...
        if inner_pixels < min_region_pixels or outer_pixels < min_region_pixels:
            # Regions too small or heavily clipped at image borders.
            continue
        ...
        # Color distance: L2 between normalized histograms.
        color_dist = float(np.linalg.norm(color_in - color_out))

        # Texture distance: L1 between log-variance + mean-edge features.
        texture_dist = float(
            np.abs(texture_in["log_var"] - texture_out["log_var"])
            + np.abs(texture_in["edge_mean"] - texture_out["edge_mean"])
        )

        # Combine per-frame color/texture distance with configurable weights.
    ...
    if valid_frames < min_valid_frames:
        # We did not see enough good frames to trust this cue.
        debug = {
    ...
    # Robust aggregate over time: median distances.
    ...
    # Map combined_median to [0,1] inconsistency score via piecewise linear.
    ...
    # Reliability: we already know we have enough frames and region sizes,
...
def _ensure_bgr_uint8(image: np.ndarray) -> Optional[np.ndarray]:
    ...
    if img.ndim == 2:
        ...
        # Assume BGR-like order (consistent with rest of pipeline).
        pass
    else:
        logger.debug(
...
def _compute_inner_outer_masks(
    ...
    if inner_x2 <= inner_x1 or inner_y2 <= inner_y1:
        # Inner box collapsed; we’ll return empty masks and let caller decide.
        return np.zeros((h, w), dtype=np.uint8), np.zeros((h, w), dtype=np.uint8)

    # Central face region: even more shrunk to avoid including the face itself.
    ...
    # Outer ring: expand bbox outward.
...
def _compute_region_stats(
    ...
    """
    # Sanity: ensure mask shape matches image.
    if mask.shape[:2] != img_bgr.shape[:2]:
        raise ValueError("Mask and image shapes do not match")

    # --- color features (HSV 2D histogram) ---
    img_hsv = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2HSV)

    # Flatten masked pixels for H and S channels.
    mask_bool = mask.astype(bool)
    if not np.any(mask_bool):
        # Caller ensures mask has pixels, but be defensive.
    ...
    if hist_sum > 1e-6:
        ...
        # Completely flat or degenerate; keep as zeros.
        pass

    # --- texture features (Laplacian edges) ---
...
def _map_dist_to_score(dist: float, low: float, high: float) -> float:
    ...
    if high <= low:
        # Degenerate config; treat any distance > low as high inconsistency.
