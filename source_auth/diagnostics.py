# source_auth/diagnostics.py
#
# Centralised formatting helpers for SourceAuth telemetry.
#
# Goal (Phase-1):
#   - Provide a single place where we convert SourceAuthScores into a
#     compact, log/telemetry-friendly string fragment.
#   - Keep the *public* function very simple today, but design it so we can
#     easily enrich the message later without touching callers.
#
# Phase-1 behaviour:
#   - Minimal fragment:
#         "src_auth:score=0.50:state=UNCERTAIN"
#
# Phase-5.2 behaviour:
#   - Rich, but still compact and backward compatible:
#
...
#
#   - Callers still treat the whole string as an opaque diagnostic fragment
#     and MUST NOT rely on the exact number or order of fields.
...
def _format_reliability_suffix(scores: SourceAuthScores) -> Optional[str]:
    ...
    if not flags:
        # Keep output minimal; we skip "rel=none" for now.
        return None
...
def format_source_auth_reason(
    ...
    """
    # Clamp score for safety; we want predictable 0–1.
    ...
    # Base prefix is guaranteed and stable.
    parts: List[str] = [
        f"src_auth:score={s:.2f}",
        f"state={state}",
    ]

    # ---------------- Component scores (3D, screen, background) --------- #
    comps = getattr(scores, "components", None)
    if comps is not None:
        # Main cue components, already in [0,1].
    ...
    # ---------------- Rich debug-derived scalars ------------------------ #
    # Motion-related: parallax / depth-like cue.
    ...
    # Screen-related: border strength, flicker.
    ...
    # Background-related: color and texture mismatch.
    ...
    # Colon-separated, as in:
    #   "src_auth:score=0.18:state=SPOOF:3d=0.12:screen=0.85:bg=0.62:..."
    return ":".join(parts)
