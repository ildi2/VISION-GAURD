# source_auth/background.py
#
# Background consistency cue for SourceAuth.
#
# Goal:
#   Given a short time window of frames for one track, estimate how consistent
#   the background "behind" the face patch is with the surrounding scene.
#
# Intuition:
#   - Real head:
#       * inside the face bbox (non-face areas: hair, shoulders, partial bg)
#         belongs to the *same physical environment* as the ring just outside.
#       * colour distributions and texture statistics are similar.
#   - Phone / screen with another room:
#       * inside the patch: background of OTHER room (on the phone),
#       * outside: your current room (desk, wall, etc.),
#       * colour/texture differ significantly.
#
# Public API:
#   compute_background_consistency(frames_window, cfg)
#
# frames_window:
#   Sequence of (image_bgr, bbox_in_frame) pairs for one track:
#       image_bgr      : np.ndarray HxWx3, BGR, uint8 or convertible
#       bbox_in_frame  : (x1, y1, x2, y2) or None
#
# cfg:
#   SourceAuthConfig-like object, or SourceAuthBackgroundConfig.
#
#   We read parameters defensively via getattr:
#
#     Top-level (usually on SourceAuthConfig):
#       - neutral_score               (default 0.5)
#       - background_min_face_px      (default 40)
#       - background_color_norm       (default 0.3)
#       - background_texture_norm     (default 20.0)
#       - background_min_frames       (default 3)
#
#     Background sub-config (cfg.background or cfg itself):
#       - inner_margin_frac           (default 0.15)
#       - outer_margin_frac           (default 0.25)
#       - min_region_pixels           (default 500)
#       - color_hist_bins             (default 32)
#       - blur_sigma_for_noise        (default 0.5)
#       - enable_noise_sharpness_hint (default True)
#       - w_color                     (default 0.7)
#       - w_texture                   (default 0.3)
#
# Returns:
#   score    ∈ [0,1]
#       0   → strong mismatch (likely phone/screen background)
#       1   → strong consistency (likely real head in same environment)
#   reliable : bool
#       False if not enough face size / pixels / frames.
#   debug    : dict
#       {
#         "color_delta": ...,
#         "texture_delta": ...,
...
#       }
#
# This module is pure and side-effect free; can be unit-tested in isolation.
...
def compute_background_consistency(
    ...
    # Background-specific sub-config (either cfg.background or cfg itself).
    bg_cfg = getattr(cfg, "background", cfg)

    # --- Top-level / global thresholds (from SourceAuthConfig) -------------
    ...
    for img_bgr, bbox in frames_window:
        ...
        if face_min < min_face_px:
            # Too small to extract meaningful inner/outer background.
            continue

        # Inner region: shrink bbox from all sides by inner_margin_frac.
        ...
        # Outer region: ring around face bbox.
        ...
        if inner_pixels < min_region_pixels or outer_pixels < min_region_pixels:
            # Regions too small → noisy statistics.
            continue

        try:
            # Color histograms (HSV) for inner vs outer.
    ...
    if valid_frames == 0:
        # No usable frames → neutral, unreliable.
        return neutral, False, {
            "color_delta": 0.0,
            "texture_delta": 0.0,
            "valid_frames": 0.0,
    ...
    # Reliability:
    #   - enough frames with good regions
    #   - total pixels reasonably above minimal per-frame threshold
...
# --------------------------------------------------------------------------- #
# Helpers: image handling and region extraction                               #
# --------------------------------------------------------------------------- #
...
def _extract_outer_ring(
    ...
    # Build top, bottom, left, right bands outside the original bbox.
    ...
    # Concatenate bands vertically; stats are global so exact layout is unimportant.
    outer_region = np.concatenate(
        [p.reshape(-1, 3) for p in patches],
        axis=0,
    ).reshape(-1, 1, 3)  # reshape to a pseudo-image (N,1,3) for hist ops

    return outer_region


# --------------------------------------------------------------------------- #
# Helpers: color / texture statistics                                         #
# --------------------------------------------------------------------------- #
...
def _color_hist_hsv(
    ...
    # If patch is reshaped (N,1,3), cvtColor still works.
    hsv = cv2.cvtColor(patch, cv2.COLOR_BGR2HSV)

    # Ranges for HSV components.
    h_range = [0, 180]
    sv_range = [0, 256]
...
def _hist_l1_distance(h1: np.ndarray, h2: np.ndarray) -> float:
    ...
    if h1.shape != h2.shape:
        # Fallback: pad/truncate to min length.
